...
2) I get auto completions with cider. E.g. ~js/..~ does something.
   ...
   #+begin_center
   BTW here is a tip, for ~sci~ projects (~babashka~, ~nbb~, ~scittle~).
...
https://i.imgur.com/M8rSuZ3.png

The word ~scittle~ makes me think of a small mammal in the habitus of
a mouse.
...
I fixed that by adding a ~play~ button.
But mobile still did not work, ok.
After checking [[https://stackoverflow.com/questions/46345883/why-audio-not-playing-on-mobile-browser][stackoverflow]] I still sort of have the idea that it has
to do with user interaction events and this ~AudioContext~.
I first wanted to delay the whole audio setup until you click the
utton. But [[https://developer.mozilla.org/en-US/docs/Web/API/AudioContext][read]] that you should make a single ~AudioContext~ at the beginning.

So I try delay the 1 other thing in the code:
